...
ds_grad = DefaultLogDataset("./analog_grad")
dl = DataLoader(ds, batch_size=16, shuffle=False, collate_fn=collate_nested_dicts)
dl_grad = DataLoader(
    ds_grad, batch_size=16, shuffle=False, collate_fn=collate_nested_dicts
)

for (data_id1, data1), (data_id2, data2) in zip(dl, dl_grad):
    for key in data1.keys():
        fwd = data1[key]["forward"]
        bwd = data1[key]["backward"]
        grad = torch.einsum("ni,nj->nij", bwd, fwd)
        grad2 = data2[key]
        assert torch.equal(grad, grad2)
    print("check done!")
