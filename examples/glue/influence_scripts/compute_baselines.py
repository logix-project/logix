...
def compute_all_trak(data_name: str, model_id: int) -> None:
    ...
    if os.path.exists(file_name):
        ...
        scores = compute_trak(
            model=model,
            expt_name=f"{data_name}_{model_id}_trak_single_model2",
            proj_dim=1024,
            checkpoints=checkpoints,
            train_loader=None,
            eval_train_loader=eval_train_loader,
...
if __name__ == "__main__":
    compute_all_trak(data_name="rte", model_id=0)
    # import sys
    #
    # try:
    ...
    #         compute_all_baselines(data_name="rte", model_id=0)
    #     elif option == 2:
    #         compute_all_trak(data_name="qnli", model_id=0)
    #         compute_all_trak(data_name="sst2", model_id=0)
    #         compute_all_trak(data_name="rte", model_id=0)
    #     elif option == 3:
    #         compute_if(data_name="qnli", model_id=0, damping=1e-08, overwrite=True)
    #         compute_if(data_name="sst2", model_id=0, damping=1e-08, overwrite=True)
    #         compute_if(data_name="rte", model_id=0, damping=1e-08, overwrite=True)
    #     else:
