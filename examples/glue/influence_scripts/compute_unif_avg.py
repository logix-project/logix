...
from src.unif.segment_avg import UnifSegmentAverageComputer
...
def compute_unif_segment(
    ...
    if os.path.exists(file_name) and not overwrite:
        ...
        computer = UnifSegmentAverageComputer(
            model=model,
            task=task,
            n_epoch=n_epoch,
            n_iters=iter_lst,
...
if __name__ == "__main__":
    ...
    compute_unif_segment(data_name="qnli", model_id=0, overwrite=False)
    compute_unif_segment(data_name="sst2", model_id=0, overwrite=False)
    compute_unif_segment(data_name="rte", model_id=0, overwrite=False)
