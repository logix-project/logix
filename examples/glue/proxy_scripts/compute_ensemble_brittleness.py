...
from experiments.glue.scripts.rte_train import train
from experiments.utils import save_tensor, set_seed

BASE_PATH = "../files/ensemble_brittleness_results_v2"
...
def main(data_name: str, algo_name_lst: List[str], model_id: int) -> None:
    ...
    for algo_name in algo_name_lst:
        ...
        if os.path.exists(file_name):
            ...
            elif "trak" in algo_name:
                algo_scores = torch.load(
                    f"../files/ensemble_results/data_{data_name}/alpha_0.5/trak_new_30.pt",
                    map_location="cpu",
                )
            else:
                # algo_scores = torch.load(
                #     f"../files/ensemble_results/data_{data_name.lower()}/model_{model_id}/{algo_name}.pt",
                #     map_location="cpu",
                # )
                algo_scores = torch.load(
...
if __name__ == "__main__":
    ...
    # main(data_name="sst2", algo_name_lst=algo_name_lst, model_id=0)
