...
def construct_model(name) -> nn.Module:
    if name == "mnist" or name == "fmnist":
        from examples.mnist_influence.pipeline import construct_model
        model = construct_model()
    elif name == "cifar10":
        from examples.cifar_influence.pipeline import construct_model
...
def get_hyperparameters(data_name: str) -> Dict[str, float]:
    ...
    if "mnist" in data_name:
        from examples.mnist_influence.pipeline import get_hyperparameters
    elif "cifar10" in data_name:
        from examples.cifar_influence.pipeline import get_hyperparameters
    else:
...
def get_loaders(
    ...
    if "mnist" in data_name:
        from examples.mnist_influence.pipeline import get_loaders
    elif "cifar10" in data_name:
        from examples.cifar_influence.pipeline import get_loaders
    else:
        raise ValueError(f"Unknown dataset: {data_name}")
