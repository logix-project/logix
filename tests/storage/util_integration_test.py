...
def generate_static_arrays():
    ...
    arrays = []
    for dt in [np.float32, np.float64, np.double, np.longdouble]:
        arrays.append(np.array([0, 1 / 2, 1 / 3, 1 / 9, 1 / 13, np.pi, np.e, np.euler_gamma], dtype=dt))
    return arrays
...
class TestMemoryMapHandler(unittest.TestCase):
    ...
    def test_write_and_read(self):
        ...
        data_buffer = [(i, {"dummy_data": arr}) for i, arr in enumerate(generate_random_arrays())]
        filename = "test_data"
        ...
        for item in metadata:
            ...
            read_data = np.frombuffer(mmap, dtype=dtype, count=size // dtype.itemsize, offset=offset).reshape(shape)
            # Test if expected value and read value equals
            self.assertTrue(np.array_equal(read_data, expected_data), "Data mismatch")

    def test_read(self):
        expected_files_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                           "test_mmap_data")
        handler = MemoryMapHandler(expected_files_path)

        filename = "test_data"
        data_buffer = [(i, {"dummy_data": arr}) for i, arr in enumerate(generate_static_arrays())]

        handler.write(data_buffer, filename)
        mmap, metadata = handler.read(filename)

        expected_mmap, _ = handler.read("expected_data.mmap")  # Using same metadata file.
        for item in metadata:
            offset = item["offset"]
            size = item["size"]
            shape = tuple(item["shape"])
            dtype = np.dtype(item["dtype"])
            test_data = np.frombuffer(mmap, dtype=dtype, count=size // dtype.itemsize, offset=offset).reshape(shape)
            expected_data = np.frombuffer(mmap, dtype=dtype, count=size // dtype.itemsize, offset=offset).reshape(shape)
